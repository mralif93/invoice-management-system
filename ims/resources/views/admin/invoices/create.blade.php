<x-layouts.admin header="Create Tax Invoice (AR)">
    <div x-data="{
        ...
        <!-- 1. Interactive A4 Tax Invoice PDF Live Preview Modal (Using Reusable modal component) -->
        <x-modal 
            show="showPdfModal" 
            title="Official Tax Invoice PDF Preview" 
            subtitle="Live Malaysian Statutory Document Rendering" 
            icon="file-text" 
            maxWidth="4xl"
        >
            <x-slot:headerAction>
            ...
            <div class="bg-slate-100 dark:bg-slate-950/50 p-4 sm:p-8 rounded-xl">
                <div class="max-w-3xl mx-auto bg-white dark:bg-slate-900 p-6 sm:p-10 rounded-xl shadow-lg border border-slate-200/80 dark:border-slate-800 text-xs text-slate-800 dark:text-slate-200 space-y-6">
        ...
        <!-- 2. Quick-Add Customer Modal (Using Reusable modal component) -->
        <x-modal 
            show="showCustomerModal" 
            title="Quick Register Customer / Patient" 
            subtitle="Adds buyer to master database without leaving invoice builder" 
            icon="user-plus" 
            maxWidth="lg"
        >
            <div class="space-y-4">
            ...
            <x-slot:footer>
                <button type="button" @click="showCustomerModal = false" class="px-3.5 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                    Cancel
                </button>
                <button type="button" @click="saveQuickCustomer()" class="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold shadow-xs transition-colors">
